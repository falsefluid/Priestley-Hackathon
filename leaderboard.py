...
def leaderboard(user_info):
    ...
    # Importing and printing the database
...
def friend_leaderboard(user_info):
    ...
    while True:
        friend = input("Enter a username to add to your friend leaderboard (or type 'done' to finish): ")
        if friend == 'done':
            break
        elif friend in users and friend not in selected_friends:
            selected_friends.append(friend)
            print(f"{friend} added to your friend leaderboard.")
        else:
            print(f"{friend} not found in the user list or already added. Please try again.")
